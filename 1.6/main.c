...
{   
    ...
    // Loop que garante a reinicialização do jogo
    ...
    {   
        ...
        // Variáveis que serão usadas durante a execução do programa
        ...
        int vencedor = 0;
        int continuar_captura_eventos = 1;

        // Loop de captura de eventos
        while (continuar_captura_eventos) 
        {   
            ...
            if (isevt) // Evento
            {
                // Finaliza o programa quando captura um evento de fechamento (ALT+F4 ou clicar no [x] da janela)
            ...
            // Informa o retângulo que chegou primeiro depois que todos atingirem a linha de chegada
            if (retangulo_teclado_chegou && retangulo_mouse_chegou && retangulo_tempo_chegou) {
                continuar_captura_eventos = 0;
                switch (vencedor) {
                    case 1:
            ...
            // Destroi as janelas anteriores antes de reiniciar
            if (!continuar_captura_eventos) {
                SDL_DestroyRenderer(ren);
                SDL_DestroyWindow(win);
            }
        }
